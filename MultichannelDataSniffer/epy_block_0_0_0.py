    ...
    def work(self, input_items, output_items):
        ...
        for i in range(len(input_items[0])):
            ...
            if input_items[0][i] == 2:  
                ...
                input_items[0][i] = 0
                conn.execute(f"INSERT INTO Log (Guid,Timestamp,Channel,Length,CurrentPacketNumber,TestId,TotalPacketCount,PayloadDataLength) VALUES ('{uuid.uuid4()}', '{timestamp}', '{self.channel}', '{input_items[0][i]}', '{0}','{0}', '{0}', '{0}' )");
